...
fn errorMessage(err: anyerror, phase: Phase) []const u8 {
    return switch (err) {
        ...
        error.CacheDirectoryChanged => "the canonical componentizer cache or one of its anchored descendants changed during the runtime build",
        ...
        error.UnsupportedZigVersion => "the selected Zig executable does not match the v0.4 toolchain pin",
...
fn errorHint(err: anyerror) ?[]const u8 {
    return switch (err) {
        ...
        error.CacheDirectoryChanged => "remove cache symlinks or replacements and retry with a stable private cache directory",
        ...
        error.UnsupportedZigVersion => "use exactly Zig 0.17.0-dev.902+7255f3e72 for top-level and nested builds",
