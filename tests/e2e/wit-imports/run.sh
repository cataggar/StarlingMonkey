...
assert_field "root add-one carries an argument and result" 33 "rec['value']" "42"
assert_field "root add-one supports repeated calls" 34 "rec['value']" "[8, 9, 10, 11, 12]"
assert_field "root-note-count starts at 0" 35 "rec['value']" "0"
assert_field "root void result is exactly undefined" 36 "rec['value']" "True"
assert_field "root-note side effect ran once" 37 "rec['value']" "1"
assert_field "root void result stays undefined on repeat" 38 "rec['value']" "True"
assert_field "root-note side effect ran twice" 39 "rec['value']" "2"
assert_field "root named aggregate recursively lowers and lifts" 40 \
  "rec['value']" "{'tag': 'accepted', 'val': {'coordinate': {'x': 6, 'y': 10}, 'labels': ['beta', 'alpha', 'host']}}"
assert_field "root use alias chain lowers and lifts through its source interface" 41 \
  "rec['value']" "{'tag': 'item', 'val': {'x': 8, 'y': 12}}"

assert_field "run-boom host trap propagates" 42 "rec['ok']" "False"
assert_field "run-boom trap message names the deliberate host error" 42 \
