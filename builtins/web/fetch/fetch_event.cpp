...
bool install(api::Engine *engine) {
  ...
  // `fetch-event` disabled (cataggar/StarlingMonkey#6 Phase 6): don't
  // register the `FetchEvent` class or wire up an incoming-request handler.
  // `addEventListener('fetch', ...)` fails deterministically at the call
  // site instead (see builtins/web/event/global-event-target.cpp). Any
  // incoming request delivered to the component's `wasi:http/incoming-
  // handler` export regardless (that export itself is generated by the
  // prebuilt host-api bindings and is outside this build's control) still
  // fails deterministically: `exports_wasi_http_incoming_handler` in
  // host_api.cpp release-asserts that a handler was installed.
  return true;
#else
